...
const PersonalCard = (props) => {
    ...
    return (
        <div id="personalCard" className={classes.personalCard + ' relative align-center w-fit bg-white md:ml-8 m-auto mt-8 h-fit xs:rounded-2xl'} style={isGreaterThanSm && isAvatarMoved ? {borderRadius: '4rem 10rem 10rem 4rem'} : {}}>
            <Grid container className='sm:py-0 py-2'>
                ...
                <Grid item xs={12} sm={9} className={ (isAvatarMoved ? '-z-10' : '') + ' flex justify-center items-center'}>
                    <div className='ml-auto mr-auto sm:ml-8 text-center sm:text-left' style={{opacity: isAvatarMoved ? 0 : 1}}>
                        <SoftTypography variant='body2' color='text' >{props.phone}</SoftTypography>
                        <SoftTypography variant='body2' color='text' >{props.email}</SoftTypography>
                        <SoftTypography variant='body2' color='text' >{props.city}</SoftTypography>
                    </div>
                </Grid>
            </Grid>
        </div>
